...
class EcmpHashFieldUDPTestV4(T0TestBase):
    """
    Verify ECMP loadbalance with different hash field, UDP protocol in this case.
    """
    ...
    def test_ecmp_hash_field_udp(self):
        """
        1. Generate Packets, with different source IPs as SIP:192.168.0.1-192.168.0.10 to match the exiting config
        2. For each case, set only one of the four ECMP hash fields (exclude SAI_NATIVE_HASH_FIELD_DST_IP)
        3. For each field just change the corresponding field, for example, for SAI_NATIVE_HASH_FIELD_IP_PROTOCOL, use UDP or TCP
        ...
        try:
            ...
            for i in range(1, 11):
                ...
                for port_index in range(0, max_itrs):
                    ...
                    exp_pkt1 = simple_udp_packet(eth_dst=self.t1_list[2][100].mac,
                                        eth_src=ROUTER_MAC,
                                        ip_dst=ip_dst,
                                        ip_src=ip_src,
                                        udp_sport= src_port,
                    ...
                    exp_pkt2 = simple_udp_packet(eth_dst=self.t1_list[3][100].mac,
                                        eth_src=ROUTER_MAC,
                                        ip_dst=ip_dst,
                                        ip_src=ip_src,
                                        udp_sport= src_port,
