    ...
    def runTest(self):
        ...
        try:
            ...
            exp_pkt = simple_tcp_packet(eth_src='00:11:11:11:11:11',
                                    ...
                                    ip_id=109,
                                    ip_ttl=64)
            print "Sending packet port 1 (lag member) -> port 5"
            send_packet(self, 1, str(pkt))
            verify_packets(self, exp_pkt, [5])
            print "Sending packet port 2 (lag member) -> port 5"
            send_packet(self, 2, str(pkt))
            verify_packets(self, exp_pkt, [5])
            print "Sending packet port 3 (lag member) -> port 5"
            send_packet(self, 3, str(pkt))
            verify_packets(self, exp_pkt, [5])
            print "Sending packet port 4 (lag member) -> port 5"
