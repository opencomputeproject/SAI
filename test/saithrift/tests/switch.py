...
def sai_thrift_create_tunnel_term_table(client, tunnel_type, addr_family, vr_id, ip_addr_dst, ip_addr_src, tunnel_oid):
    ...
    if addr_family == SAI_IP_ADDR_FAMILY_IPV4 :
         ...
         ipprefix_dst = sai_thrift_ip_prefix_t(addr_family = SAI_IP_ADDR_FAMILY_IPV4 ,addr=addr)
    elif addr_family == SAI_IP_ADDR_FAMILY_IPV6:
         addr = sai_thrift_ip_t(ip6=ip_addr_dst)
         mask = sai_thrift_ip_t(ip6='ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff')
         ipprefix_dst = sai_thrift_ip_prefix_t(addr_family = SAI_IP_ADDR_FAMILY_IPV6 ,addr=addr)
