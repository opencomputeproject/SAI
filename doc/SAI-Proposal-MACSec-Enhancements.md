...
- **MACsec security modes** Configuration of security modes that define the behavior of MACsec protection on a given link when a MACsec Key Agreement (MKA) session is not established.
...
Must Secure is the most stringent mode.
...
The below MACsec port attribute is newly introduced to allow configuration of the MACsec security mode. This attribute controls how the switch’s MACsec security engine enforces link protection. When set, the attribute instructs the hardware to apply the corresponding security mode on the specified port.
...
The Security Mode is defined to take values of the below enumeration.
...
- **Step 3** Set Security Mode as part of Create MACsec Port.
...
```c
    ...
    /* If should secure, set mode appropriately */
    if (should_secure) {
