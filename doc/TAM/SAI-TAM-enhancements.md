...
This proposal enhances SAI TAM 2.0 by adding new reporting modes (sampling, microburst), a new transport type (GRE), header encapsulation attributes, and collector destination options (port/lag).
...
- New TAM report modes and attributes to support the generation of report sampling and microburst reports.
...
- Switch attribute to control the set of queues where the switch TAM policy will be applied.
...
Sending all the report events can overwhelm the collector.Probabilistic sampling reduces the load on the collector by only sending
 1 in every N events.

```c
+    /** Report in a probabilistic sampling mode, one report is sent in n events */
+    SAI_TAM_REPORT_MODE_PROBABILISTIC_SAMPLING,
+
```
#### Microburst mode
When multiple concurrent flows cause a microburst, it is useful to report all the packets in the microburst. This can be achieved by using the microburst mode.
```c
+    /** Report in a microburst mode, specifying report rate and max number of reports per microburst event */
+    SAI_TAM_REPORT_MODE_MICROBURST,
+
...
```c
+    /**
+     * @brief Probabilistic sampling rate 'n' for probabilistic sampling mode
+     *
+     * @type sai_uint32_t
+     * @flags MANDATORY_ON_CREATE | CREATE_AND_SET
+     * @condition SAI_TAM_REPORT_ATTR_REPORT_MODE == SAI_TAM_REPORT_MODE_PROBABILISTIC_SAMPLING
+     */
+    SAI_TAM_REPORT_ATTR_PROBABILISTIC_SAMPLE_RATE,
+
+    /**
+     * @brief Report rate in packets per second when in microburst mode
+     *
+     * @type sai_uint32_t
+     * @flags MANDATORY_ON_CREATE | CREATE_AND_SET
+     * @condition SAI_TAM_REPORT_ATTR_REPORT_MODE == SAI_TAM_REPORT_MODE_MICROBURST
+     */
+    SAI_TAM_REPORT_ATTR_MICROBURST_RATE,
+
+    /**
+     * @brief Maximum number of reports per burst when in microburst mode
+     *
+     * @type sai_uint32_t
+     * @flags MANDATORY_ON_CREATE | CREATE_AND_SET
+     * @condition SAI_TAM_REPORT_ATTR_REPORT_MODE == SAI_TAM_REPORT_MODE_MICROBURST
+     */
+    SAI_TAM_REPORT_ATTR_MAX_REPORTS_PER_MICROBURST,
+
...
### Apply the TAM object linked to the switch to the specified set of queues.
```c
+    /**
...
```
3. Create a report object with probabilisitic sampling mode
```c
    count = 0;
    memset(attr_list, 0, sizeof(attr_list));
    ...
    attr_list[count].value.s32 = SAI_TAM_REPORT_MODE_PROBABILISTIC_SAMPLING;
    count++;
    attr_list[count].id = SAI_TAM_REPORT_ATTR_PROBABILISTIC_SAMPLE_RATE;
...
```
8. Bind the TAM to the switch and specify the set of queues where this TAM policy is applicable.
```c
    uint8_t queue_list1[6] = { 2, 3, 4, 5, 6};

    memset(attr_list, 0, sizeof(attr_list));
    attr_list[0].id = SAI_SWITCH_ATTR_TAM_QUEUE_INDEX_LIST;
    attr_list[0].value.u8list.count = 1;
    attr_list[0].value.u8list.list  = queue_list;

    rc = switch_api_p->set_switch_attribute(switch_id, &attr_list[0]);

    /* Bind TAM to Switch object */
    count = 0;
    memset(attr_list, 0, sizeof(attr_list));
    attr_list[count].id = SAI_SWITCH_ATTR_TAM_OBJECT_ID;
    attr_list[count].value.objlist.count = 1;
    attr_list[count].value.objlist.list = (sai_object_id_t *)malloc(1*sizeof(sai_object_id_t));
    attr_list[count].value.objlist.list[0] = tam_id;
    count++;

    rc = switch_api_p->set_switch_attribute(switch_id, &attr_list[0]);

```
#### Microsburst tam report object
Example flow to create microburst report at rate=512 (pps), max reports (packets) per burst=1024 (packets).
...
```c
    ...
    attr_list[count].id = SAI_TAM_REPORT_ATTR_REPORT_MODE;
    attr_list[count].value.s32 = SAI_TAM_REPORT_MODE_MICROBURST;
    count++;
    attr_list[count].id = SAI_TAM_REPORT_ATTR_MICROBURST_RATE;
    attr_list[count].value.u32 = 512;
    count++;
    attr_list[count].id = SAI_TAM_REPORT_ATTR_MAX_REPORTS;
...
```
#### Example workflow for applying a TAM policy to an ACL flow.
Replace above step (8) with below api sequence.
```c
