...
JsonObjects::add_header_type(const cstring& name, Util::JsonArray*& fields, unsigned max_length) {
    ...
    unsigned id = SAI::nextId("header_types");
...
JsonObjects::add_union_type(const cstring& name, Util::JsonArray*& fields) {
    ...
    unsigned id = SAI::nextId("header_union_types");
...
cstring JsonObjects::get_header_type(Util::JsonValue *hdr)
{
    if (hdr != nullptr) {
        const cstring protocol = hdr->getString();
        Util::JsonObject *header = find_object_by_name(headers, protocol);
        if (header != nullptr) {
            // This is the header with "name" == hdr->getString()
            // Now find the header type
            auto header_type = header->get("header_type")->to<Util::JsonValue>();
            if (header_type != nullptr) {
                // Find the field in the type header list
                cstring hdr_type_name = header_type->getString();
                return hdr_type_name;
            }
        }
    }
    return nullptr;
...
/// from a specified header and field within that header, find and return
/// the size and offset of that field, in bits
/// FIXME: ALAN find_header_field
bool JsonObjects::find_header_field(Util::JsonValue *hdr, Util::JsonValue * val,
        uint32_t &field_offset, uint32_t &field_size, bool &field_is_signed, bool &is_sai_extracted) {
    cstring hdr_type_name = get_header_type(hdr);
    if (!hdr_type_name.isNullOrEmpty()) {
        // Find the field in the type header list
        auto header_type = find_object_by_name(header_types, hdr_type_name);
        if (header_type != nullptr) {
            // find the field
            if (val != nullptr) {
                cstring value_name = val->getString();
                // filter out synthetic fields like $valid$
                if (!value_name.startsWith("$")) {
                    auto fields = header_type->get("fields")->to<Util::JsonArray>();
                    // fields is an array of 3 items, name, size and is_signed
                    field_offset = 0;
                    field_size = 0;
                    for (auto entry : *fields) {
                        Util::JsonArray *field_entry = entry->to<Util::JsonArray>();
                        auto name = field_entry->at(0)->to<Util::JsonValue>();
                        auto size = field_entry->at(1)->to<Util::JsonValue>();
                        if (name->getString() == value_name) {
                            // FIXME - ALAN need to use CB only if this is not a pre-defined extraction
                            field_size = size->getInt();
                            auto is_signed = field_entry->at(2)->to<Util::JsonValue>();
                            field_is_signed = is_signed->getBool();
                            auto extract = field_entry->at(3)->to<Util::JsonValue>();
                            is_sai_extracted = extract->getBool();
                            break;
                        }
                        field_offset += size->getInt();
                    }
                    // convert from bits to bytes
                    field_offset = field_offset >> 3;
                    field_size = ((field_size-1) >> 3) + 1;
                    return true;
                }
            }
        }
...
/// The header type is described by the name.
void
...
unsigned
JsonObjects::add_header(const cstring& type, const cstring& name, bool is_predefined) {
    auto header = new Util::JsonObject();
    unsigned id = SAI::nextId("headers");
    ...
    header->emplace("sai_predefined", is_predefined);
...
JsonObjects::add_union(const cstring& type, Util::JsonArray*& headers, const cstring& name) {
    ...
    unsigned id = SAI::nextId("header_unions");
...
JsonObjects::add_parser_state(const unsigned parser_id, const cstring& state_name) {
    ...
    unsigned state_id = SAI::nextId("parse_states");
...
JsonObjects::add_parse_vset(const cstring& name, const unsigned size) {
    ...
    unsigned id = SAI::nextId("parse_vsets");
...
JsonObjects::add_action(const cstring& name, Util::JsonArray*& params, Util::JsonArray*& body) {
    ...
    unsigned id = SAI::nextId("actions");
...
JsonObjects::add_extern(const cstring& name, const cstring& type,
    ...
    unsigned id = SAI::nextId("extern_instances");
