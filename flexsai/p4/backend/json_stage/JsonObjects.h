...
#ifndef _BACKENDS_SAI_JSONOBJECTS_H_
#define _BACKENDS_SAI_JSONOBJECTS_H_
...
class JsonObjects {
 public:
    ...
    cstring get_header_type(Util::JsonValue *hdr);
    bool find_header_field(Util::JsonValue *jv, Util::JsonValue * jv3,
            uint32_t &offset, uint32_t &size,  bool &field_is_signed, bool &is_sai_extracted);
    void add_header_field(const cstring& name, Util::JsonArray*& field);
    unsigned add_header(const cstring& type, const cstring& name, bool is_predefined);
...
#endif /* _BACKENDS_SAI_JSONOBJECTS_H_ */
