...
cstring ControlConverter::getKeyMatchType(const IR::KeyElement *ke) {
    ...
    if (mt->name.name == backend->getCoreLibrary().exactMatch.name ||
        ...
        backend->getSaiSwitch()->find_match_kind(mt->name.name)) {
        return mt->name.name;
    }
...
ControlConverter::handleTableImplementation(const IR::Property* implementation,
    ...
    if (propv->expression->is<IR::ConstructorCallExpression>()) {
        ...
        if (implementationType->name == SAI::TableImplementation::actionSelectorName) {
            ...
            for (auto ke : key->keyElements) {
                ...
                BUG_CHECK(mt != nullptr, "%1%: could not find declaration", ke->matchType);
                if (mt->name.name != SAI::MatchImplementation::selectorMatchTypeName)
                    continue;
        ...
        } else if (implementationType->name == SAI::TableImplementation::actionProfileName) {
            isSimpleTable = false;
        ...
        if (type_extern_name == SAI::TableImplementation::actionProfileName) {
            table->emplace("type", "indirect");
        } else if (type_extern_name == SAI::TableImplementation::actionSelectorName) {
            table->emplace("type", "indirect_ws");
        } else {
            ::error("%1%: unexpected type for implementation", dcltype);
            return false;
...
//            backend->getSaiSwitch()->convertExternInstances(decl->to<IR::Declaration>(),
...
ControlConverter::convertTable(const CFG::TableNode* node,
    ...
    // Mark specific tables that are FLEX type
    // TODO - ALAN Add annotation for hw vs flex tables
    // We need to separate ACL table generation from P4 table ID generation.
    // Always need the Table ID in the P4 info
    const IR::Annotation* annotation = table->getAnnotation(SAI_FIXED_TABLE);
    bool is_flex_table = true;
    if (annotation != nullptr && annotation->name.name == SAI_FIXED_TABLE) {
        is_flex_table = false;
    }
    result->emplace(SAI_FLEX_TABLE, is_flex_table);
    ...
    if (key != nullptr) {
        for (auto ke : key->keyElements) {
            ...
            if (match_type == SAI::MatchImplementation::selectorMatchTypeName)
                    continue;
            ...
            if (match_type != table_match_type) {
                if (match_type == SAI::MatchImplementation::rangeMatchTypeName)
                    table_match_type = SAI::MatchImplementation::rangeMatchTypeName;
            ...
            auto jk = conv->convert(expr);
            auto value = jk->to<Util::JsonObject>()->get("value");
            keyelement->emplace("target", value);

            // TODO ALAN table insert custom byte offset and size to table key
            Util::JsonObject *payload = correlateHeader(value);
            if (payload != nullptr)
                keyelement->emplace("sai_extract", payload);
    ...
    auto impl = table->properties->getProperty(SAI::TableAttributes::implementationName);
    bool simple = handleTableImplementation(impl, key, result, action_profiles, selector_check);

    unsigned size = 0;
    auto sz = table->properties->getProperty(SAI::TableAttributes::sizeName);
    if (sz != nullptr) {
    ...
    if (size == 0)
        size = SAI::TableAttributes::defaultTableSize;

    result->emplace("max_size", size);
    auto ctrs = table->properties->getProperty(SAI::TableAttributes::countersName);
    if (ctrs != nullptr) {
        ...
        if (ctrs->value->is<IR::ExpressionValue>()) {
            ...
            if (expr->is<IR::ConstructorCallExpression>()) {
                ...
                if (te->name != SAI::TableImplementation::directCounterName &&
                    te->name != SAI::TableImplementation::counterName) {
                    ::error("%1%: Unexpected type %2% for property", ctrs, type);
                    return result;
                }
                ...
                bool direct = te->name == SAI::TableImplementation::directCounterName;
    ...
    bool sup_to = false;
    auto timeout = table->properties->getProperty(SAI::TableAttributes::supportTimeoutName);
    if (timeout != nullptr) {
    ...
    auto dm = table->properties->getProperty(SAI::TableAttributes::metersName);
    if (dm != nullptr) {
        if (dm->value->is<IR::ExpressionValue>()) {
            ...
            if (!expr->is<IR::PathExpression>()) {
                ...
                auto te = type->to<IR::Type_Extern>();
                if (te->name != SAI::TableImplementation::directMeterName) {
                    ::error("%1%: Unexpected type %2% for property", dm, type);
                    return result;
                }
                if (!decl->is<IR::Declaration_Instance>()) {
...
/// For the given json object, find the control "target" attribute
/// and correlate it with the header and field value by finding
/// the offset and byte size in the payload
/// This allows the code generation to use a custom byte offset and size
/// for a table key
// TODO ALAN - do not hard code UDP payload
Util::JsonObject* ControlConverter::correlateHeader(Util::IJson *value) {
    if (value != nullptr && value->is<Util::JsonArray>()) {
        // Target should consist of a protocol header and field value
        Util::JsonArray *list = value->to<Util::JsonArray>();
        auto hdr = list->at(0)->to<Util::JsonValue>();
        auto field = list->at(1)->to<Util::JsonValue>();
        uint32_t offset = 0;
        uint32_t size = 0;
        bool is_signed = false;
        bool is_pre_extracted = false;
        if (json->find_header_field(hdr, field, offset, size, is_signed, is_pre_extracted)
            && !is_pre_extracted) {
            auto payload = new Util::JsonObject();
            payload->emplace("header", hdr->getString());
            payload->emplace("header_type", json->get_header_type(hdr));
            payload->emplace("field_offset", offset);
            payload->emplace("field_size", size);
            // Extraction point must be on a even byte boundary
            // FIXME - ALAN changes for Sai 2?
            if ((offset % 2) != 0) {
                offset--;
                size++;
            }
            payload->emplace("extract_offset", offset);
            payload->emplace("extract_size", size);
            payload->emplace("signed", is_signed);
            return payload;
        }
    }
    return nullptr;
