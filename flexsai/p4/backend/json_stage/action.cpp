    ...
    for (auto s : *body) {
        ...
        if (!s->is<IR::Statement>()) {
            ...
            if (mi->is<P4::ActionCall>()) {
                ...
                BUG("sai: unimplemented action extern method");
            }
            //------------------------------
            // sai action list hadeling
            //------------------------------
            else if(mi->is<P4::ExternFunction>()){
                auto extrn = mi->to<P4::ExternFunction>();
                cstring prim = mc->toString();
                auto params = new Util::JsonArray();
                // parameter list to json, brings back function definition and not function instance:
                // convertActionParams(mc->getActualParameters(), params);
                // below code passes references to extern function.
                for (uint i=0; i< mc->arguments->size(); ++i){
                    auto obj = conv->convert(mc->arguments->at(i));
                    params->append(obj);
                }
                auto primitive = mkPrimitive(prim, result);
                primitive->emplace("parameters", params);
                primitive->emplace_non_null("source_info", s->sourceInfoJsonObj());
                continue;
            }
            else{
                ::error("sai: unclassified methodCall: %1%",s);
                continue;
            }
        }
