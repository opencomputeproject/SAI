...
void ConvertHeaders::addHeaderStacks(const IR::Type_Struct* headersStruct) {
    ...
    for (auto f : headersStruct->fields) {
        ...
        for (unsigned i = 0; i < stack_size; i++) {
            ...
            auto id = json->add_header(stack_type, hdrName, isSaiFixedHeader(hdrName, ft));
            ids.push_back(id);
        }
...
void ConvertHeaders::addHeaderField(const cstring& header, const cstring& name,
                                    int size, bool is_signed, bool is_sai_extracted) {
    Util::JsonArray* field = new Util::JsonArray();
    field->append(name);
    field->append(size);
    field->append(is_signed);
    field->append(is_sai_extracted);
...
void ConvertHeaders::addHeaderType(const IR::Type_StructLike *st) {
    ...
    for (auto f : st->fields) {
        ...
        // FIXME - ALAN attributes on fields
        bool isExtracted = isSaiExtractedField(name, f);
    ...
    if (padding != 0) {
        ...
        field->append(false);
        field->append(false); // NOT a SAI extracted field
    }
...
Visitor::profile_t ConvertHeaders::init_apply(const IR::Node* node) {
    ...
    for (auto v : backend->getStructure().variables) {
        ...
        if (v->is<IR::StructField>()) {
            is_extracted = isSaiExtractedField(v->toString(), v->to<IR::StructField>());
        }
        auto type = typeMap->getType(v, true);
        if (auto st = type->to<IR::Type_StructLike>()) {
            auto metadata_type = st->controlPlaneName();
            if (type->is<IR::Type_Header>())
                json->add_header(metadata_type, v->name, isSaiFixedHeader(metadata_type, type));
            else
            ...
            for (unsigned i=0; i < stack->getSize(); i++) {
                ...
                auto header_id = json->add_header(header_type, name, isSaiFixedHeader(name, type));
            ...
            addHeaderField(scalarsTypeName, v->name.name, tb->size, tb->isSigned, is_extracted);
            scalars_width += tb->size;
        } else if (type->is<IR::Type_Boolean>()) {
            addHeaderField(scalarsTypeName, v->name.name, boolWidth, false, is_extracted);
            scalars_width += boolWidth;
        } else if (type->is<IR::Type_Error>()) {
            addHeaderField(scalarsTypeName, v->name.name, errorWidth, 0, is_extracted);
            scalars_width += errorWidth;
...
void ConvertHeaders::end_apply(const IR::Node*) {
    ...
    if (padding != 0) {
        cstring name = refMap->newName("_padding");
        addHeaderField(scalarsTypeName, name, 8-padding, false, false);
    }
}
...
// FIXME- ALAN annotations to headers
bool ConvertHeaders::isSaiFixedHeader(const cstring& name, const IR::Type *ft) {
    const IR::Annotation* annotation = ft->getAnnotation("sai_fixed");
    if (annotation != nullptr && Log::verbose()) {
        std::cout << name << " : " << ft->node_type_name() << " is SAI_FIXED with ";
        for (auto field : annotation->expr) {
            std::cout << field->toString() << ", ";
        }
        std::cout << std::endl;
    }
    return (annotation != nullptr) ? true : false;
}

// FIXME- ALAN annotations to header fields
bool ConvertHeaders::isSaiExtractedField(const cstring& name, const IR::StructField *ft) {
    const IR::Annotation* annotation = ft->getAnnotation("sai_extract");
    if (annotation != nullptr && Log::verbose()) {
        std::cout << name << " : " << ft->name.name << " is SAI_EXTRACT with ";
        for (auto field : annotation->expr) {
            std::cout << field->toString() << ", ";
        }
        std::cout << std::endl;
