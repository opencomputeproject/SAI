...
expression removed.
*/
class ProcessControls : public SAI::RemoveComplexExpressionsPolicy {
...
void
Backend::process(const IR::ToplevelBlock* tlb, SaiOptions& options) {
    CHECK_NULL(tlb);
    setName("BackEnd");
    ...
    saiSwitch->setPipelineControls(tlb, &pipeline_controls, &pipeline_namemap);
    saiSwitch->setParserControls(tlb, &parser_controls);
    saiSwitch->setDeparserControls(tlb, &deparser_controls);
    
    // Find the user metadata declaration
    auto parser = saiSwitch->getParser(tlb);
    ...
    addPasses({
        ...
        new P4::SynthesizeActions(refMap, typeMap, new SkipControls(&deparser_controls)),
...
void Backend::convert(SaiOptions& options) {
...
bool Backend::isStandardMetadataParameter(const IR::Parameter* param) {
    auto parser = saiSwitch->getParser(getToplevelBlock());
    auto params = parser->getApplyParameters();
    if (params->size() != 4) {
        saiSwitch->modelError("%1%: Expected 4 parameter for parser", parser);
        return false;
    }
    if (params->parameters.at(3) == param)
        return true;

    auto ingress = saiSwitch->getIngress(getToplevelBlock());
    params = ingress->getApplyParameters();
    if (params->size() != 4) {
        saiSwitch->modelError("%1%: Expected 4 parameter for ingress", ingress);
        return false;
    }
    if (params->parameters.at(2) == param)
        return true;

    auto egress = saiSwitch->getEgress(getToplevelBlock());
    params = egress->getApplyParameters();
    if (params->size() != 4) {
        saiSwitch->modelError("%1%: Expected 4 parameter for egress", egress);
        return false;
    }
    if (params->parameters.at(2) == param)
        return true;
...
bool Backend::isUserMetadataParameter(const IR::Parameter* param) {
        ...
        auto params = parser->getApplyParameters();
        if (params->size() != 4) {
            saiSwitch->modelError("%1%: Expected 4 parameters for parser", parser);
            return false;
        }
        if (params->parameters.at(2) == param)
            return true;
        ...
        params = ingress->getApplyParameters();
        if (params->size() != 4) {
            saiSwitch->modelError("%1%: Expected 4 parameters for ingress", ingress);
            return false;
        }
        ...
        params = egress->getApplyParameters();
        if (params->size() != 4) {
            saiSwitch->modelError("%1%: Expected 4 parameters for egress", egress);
            return false;
        }
        if (params->parameters.at(1) == param)
            return true;
