...
_In_ const service_method_table_t* services)
{
	...
	{
		REDIS_LOG_ERR(stderr, "Invalid services handle passed to SAI API initialize\n");
	...
	if (0 != flags)
	{
		REDIS_LOG_ERR(stderr, "Invalid flags passed to SAI API initialize\n");
		return SAI_STATUS_INVALID_PARAMETER;
	}

	// TODO create map sai_redis_type_map_initialize();
...
sai_status_t sai_log_set(_In_ sai_api_t sai_api_id, _In_ sai_log_level_t log_level)
{
	...
	default:
		REDIS_LOG_ERR(stderr, "Invalid log level %d\n", log_level);
		return SAI_STATUS_INVALID_PARAMETER;
	}
	...
	default:
		REDIS_LOG_ERR(stderr, "Invalid API type %d\n", sai_api_id);
		return SAI_STATUS_INVALID_PARAMETER;
	}
...
sai_status_t sai_api_query(_In_ sai_api_t sai_api_id, _Out_ void** api_method_table)
{
	...
	{
		REDIS_LOG_ERR(stderr, "NULL method table passed to SAI API initialize\n");
		return SAI_STATUS_INVALID_PARAMETER;
	}

	if (!g_initialized) 
	{
		REDIS_LOG_ERR(stderr, "SAI API not initialized before calling API query\n");
