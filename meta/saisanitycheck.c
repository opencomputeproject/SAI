        ...
        _In_ const sai_enum_metadata_t* emd)
{
    ...
    if ((emd->values[0] & 0xFFF) == 0)
    {
        /* enum starts from 0 or range, ok, it can contain ranges */
    ...
    {
        /* enum starts from 1, ok, it must be flags */

        return;
    }

    META_ASSERT_FAIL("enum %s starts from %d but should start from 0 or 1", emd->name, emd->values[0]);
}
