        ...
        _In_ const sai_attr_metadata_t* metadata)
{
    ...
    if (metadata->objecttype == SAI_OBJECT_TYPE_ACL_ENTRY)
    {
        ...
        if (metadata->attrid >= SAI_ACL_ENTRY_ATTR_ACTION_CUSTOM_RANGE_BASE &&
            metadata->attrid < SAI_ACL_ENTRY_ATTR_ACTION_CUSTOM_RANGE_BASE + 0x1000)
        {
            return true;
        }

        if (metadata->attrid >= SAI_ACL_ENTRY_ATTR_FIELD_CUSTOM_RANGE_BASE &&
            metadata->attrid < SAI_ACL_ENTRY_ATTR_FIELD_CUSTOM_RANGE_BASE + 0x1000)
        {
            return true;
        }
    ...
    switch (md->attrvaluetype)
    {
        ...
        case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_UINT8_LIST:
            ...
            if (md->objecttype == SAI_OBJECT_TYPE_ACL_ENTRY &&
                    ...
                    md->attrid <= SAI_ACL_ENTRY_ATTR_FIELD_CUSTOM_RANGE_BASE + 0x1000)))
            {
                break;
...
void check_acl_entry_actions()
{
    ...
    while (meta_acl_entry[index] != NULL &&
           ...
           meta_acl_entry[index]->attrid < SAI_ACL_ENTRY_ATTR_ACTION_CUSTOM_RANGE_BASE + 0x1000) {
        const sai_attr_metadata_t *meta = meta_acl_entry[index];
